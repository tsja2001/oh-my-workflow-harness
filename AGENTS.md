...
| `ai-docs/` | AI 自维护公用知识：`工作规范与习惯.md`（活文档，必读）、`creds.env`（凭据，**禁止提交**） | AI |
...
- 公司代码仓库纪律：新需求从 prod 拉 `feature/xxx`；未上线功能的补丁从 test 基线拉子分支；开发完 merge --no-ff 进 test。不 force push 保护分支、不 `mvn deploy`、不动 K8s 部署。
