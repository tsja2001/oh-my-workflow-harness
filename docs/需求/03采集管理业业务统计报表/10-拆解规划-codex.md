...
> 日期：2026-07-20
> 工具：codex
> 当前状态：ES 写入、report 查询/导出和前端基础链路已跑通；2026-07-27 已把“采购业务类型”下拉从前端写死两项改为读取公共字典 `purchase_business_type`，仍传中文名称匹配 ES，前端构建通过，按用户要求未提交。
> 下一阶段：由用户决定何时提交并部署前端；部署后验收采购业务类型下拉与字典四项同步，并分别验证“集团集采/分散采购”查询。
>
...
27. ✅ 2026-07-27 采购业务类型下拉改为调用现成公共字典 `purchase_business_type`：
    - 删除前端写死的“集团集采/分散采购”，页面加载时读取字典四项。
    - 字典接口返回 `value=101/102/103/104`，但 ES 存的是中文名称，因此下拉查询值使用 `label`，继续向 report 后端传中文字符串。
    - 不改 report 后端、ES mapping 和五来源同步逻辑；`npm run build` 通过，按用户要求未提交。
