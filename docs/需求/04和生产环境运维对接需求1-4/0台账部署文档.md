...
- [ procurementScheme ]       scm-vue-statics-pro
  - 同事部分（日金额/日计划/价格趋势 3 页）：分支 `prod-feature-jcmh`（已交部署负责人）
  - 杨卓然部分（坑口日指标 1 页 + 路由）：分支 `taizhang-prod-yang`（本地基于 origin/prod@997a86f7，单提交 `57d25dc6`，未推送）

后端
scm-source-all(分支 feature/jicai-taizhang-prod-yang)
scm-order-all（分支 uat）


1. 配置4个导出模板
集采日金额统计台账：collection-daily-ledger-export 集采日金额统计台账导出
集采日计划统计台账: centralPurchaseStatistics 集采日统计视图对象
煤炭重点坑口日指标: coal-pit-daily-export 煤炭重点坑口日指标台账导出
价格趋势维护： priceTrend 价格趋势维护导出

2. 配置菜单
參考test环境-集采业务管理台账 下面四个菜单
