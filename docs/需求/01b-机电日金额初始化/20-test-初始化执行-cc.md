...
> 日期：2026-09-04 ｜ 工具：cc ｜ 状态：**SQL 已生成待执行**（预检未跑、未灌库、未同步） ｜ 下一步：按下面 5 步在 test 跑通，数字对上后复制一套给 uat
>
...
四节期望（详见 SQL 内注释）：
① 8 个统计日流水号中段占用数全为 0；② 六家组织在 test 库存在且 BU 正确；③ Owningplate 字典六个短名齐全；④ 记下「灌库前基线」两个数字（约 16 行 / 433,923.93 元，是历史测试数据，属正常）。
...
| 全部机电行 | 97 行 / 129,174,863.02（= 初始化 81 + 历史测试 16） |
...
期望：count = 97，sum ≈ 129,174,863.02（= MySQL ② 号口径）。
...
- [ ] ES count=97 sum≈129,174,863.02（＿＿＿）
- [ ] 驾驶舱四格增量对上（＿＿＿）
