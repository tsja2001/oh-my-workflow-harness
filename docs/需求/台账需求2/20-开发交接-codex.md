...
> 日期：2026-07-10 ｜ 工具：codex ｜ 状态：后端代码、test 数据库和导出配置均完成；代码已合入并推送 test，待 Jenkins 部署
> 下一阶段：用户点 Jenkins 并完成菜单岗位配置；测试 AI 按 `21-接口文档-codex.md` 跑新增→查询→详情→修改→驾驶舱→删除→回退→导出链路

## 1. 本次改了什么

**改动简报（给用户）：** 在寻源 source 服务新增了“煤炭重点坑口日指标台账”的完整后端，支持分页查询、录入即发布、修改、逻辑删除、详情和 Excel 导出。数据采用“主表 + 坑口明细表”，当前页面支持 5 组坑口，以后增加数量不用搬表。驾驶舱原接口地址和返回字段不变，但 `kengk` 已改为实时读取最新有效台账，不再读取旧缓存。现有其他驾驶舱分支未改。
...
- 驾驶舱没有缓存兜底：新台账所有有效记录都删光时返回空列表，这是同事确认的最终口径。
